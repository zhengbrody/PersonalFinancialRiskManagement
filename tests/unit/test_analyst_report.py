...
def test_fetch_analyst_report_data_assembles_structure(monkeypatch):
    ...
    def fake_get(url, params=None, timeout=None, headers=None):
        ...
        if url.endswith("/income-statement"):
            return _mock_fmp_response(
                [{"date": "2026-03-31", "revenue": 2.5e10, "netIncome": 1.2e10}]
            )
        ...
        if url.endswith("/key-metrics"):
            return _mock_fmp_response(
                [{"date": "2026-03-31", "peRatio": 60, "enterpriseValueOverEBITDA": 45}]
            )
        ...
        if url.endswith("/grades-historical"):
            return _mock_fmp_response(
                [
                    {
                        "publishedDate": "2026-04-10T00:00",
...
def _valid_report_json():
    return {
        ...
        "street_consensus_diff": {
            "street_target": 1080,
            "our_target": 1100,
            "direction": "above_street",
            "differentiation": "More bullish on data center mix",
        },
        "catalysts_next_12m": ["Q2 earnings", "Blackwell launch"],
        "top_bank_views": [
            {"bank": "Goldman Sachs", "rating": "Buy", "target": 1250, "stance": "conviction"}
        ],
    }
...
def test_generate_report_returns_parsed_json(monkeypatch):
    ...
    def fake_get(url, params=None, timeout=None, headers=None):
        if url.endswith("/profile"):
            return _mock_fmp_response(
                [{"companyName": "NVIDIA", "sector": "Tech", "industry": "Semis"}]
            )
        if url.endswith("/quote"):
    ...
    class _FakeClient:
        def __init__(self, api_key):
            ...
            self.messages.create = MagicMock(
                return_value=_FakeResp(json.dumps(_valid_report_json()))
            )
