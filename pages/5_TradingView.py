...
from ui.components import render_section, render_kpi_row, render_metric_list
from ui.tradingview import (
    render_advanced_chart, render_mini_chart, render_technical_analysis_widget,
    render_screener_widget, render_ticker_tape, render_heatmap,
    render_fullpage_tradingview, render_fullpage_tradingview_with_watchlist,
    get_ta_summary, get_portfolio_ta, get_tv_symbol, EXCHANGE_MAP, _CRYPTO_TV,
)
...
if not weights:
    st.info(
        ...
        if lang == "en" else
        "💡 暂未加载投资组合。你仍可浏览下方的图表、筛选器和热力图。在侧边栏配置组合并点击 **Run Analysis** 即可优先查看自己的持仓。"
    )
...
with tab_chart:
    ...
    with col_interval:
        interval = st.selectbox(
            ...
            format_func=lambda x: {"1": "1m", "5": "5m", "15": "15m", "60": "1H",
                                   "240": "4H", "D": "1D", "W": "1W", "M": "1M"}.get(x, x),
            label_visibility="collapsed",
        )
    ...
    with col_data:
        ...
        if ta_data and "error" not in ta_data:
            ...
            rec_color = {
                ...
                "STRONG_SELL": "negative", "SELL": "negative",
            }.get(rec, "neutral")

            render_kpi_row([
                {"label": "Overall Signal", "value": rec.replace("_", " "),
    ...
    if weights:
        ...
        if st.button("Scan All Holdings", key="tv_scan_all", type="primary"):
            ...
            for ta in portfolio_ta:
                if "error" in ta:
                    rows.append({
                        "Ticker": ta["ticker"], "Signal": "ERROR",
                        "Oscillators": "-", "Moving Avgs": "-",
                        "RSI": "-", "MACD": "-", "ADX": "-",
                    })
                else:
                    ind = ta["indicators"]
                    rows.append({
                        ...
                        "RSI": ind["RSI"], "MACD": ind["MACD"], "ADX": ind["ADX"],
                    })
...
with tab_screener:
    ...
    screen_type = st.selectbox(
        "Screen Type",
        ["most_capitalized", "volume_leaders", "top_gainers", "top_losers",
         "ath", "atl", "above_52wk_high", "below_52wk_low"],
