...
describe("MarketsGate", () => {
  it("anonymous → marketing intro + live public preview + sign-in CTA (no app-only bits)", () => {
    ...
    expect(screen.queryByText(/advanced Markets workbench/i)).not.toBeInTheDocument();
  });

  it("signed-in → the full live desk (sentiment + workbench link), not the intro", () => {
    ...
    expect(screen.getByText(/advanced Markets workbench/i)).toBeInTheDocument();
